...
public class MenuRankingService {
    ...
    /**
     * Concurrent USDA lookups per scan — enough to hide the latency, low enough
     * to stay a polite client. Configurable (app.menu-resolve-parallelism)
     * rather than a constant, because the right number depends on the USDA key's
     * rate limit, which differs per deployment.
     */
    private final ExecutorService nutritionPool;
    ...
    public MenuRankingService(VisionAnalysisClient visionClient, ScoringService scoringService,
        ...
        this.nutritionPool = Executors.newFixedThreadPool(
                Math.max(1, props.getMenuResolveParallelism()), runnable -> {
    ...
    private List<FoodItem> resolveInParallel(List<IdentifiedFood> identified) {
        ...
        for (IdentifiedFood cf : identified) {
            pending.add(nutritionPool.submit(() -> analysisService.resolveNutrition(cf, true)));
        }
    ...
    public List<MenuHistoryEntry> history(Long userId) {
        // Projection, not entities: menu_scan carries result_json and a base64
        // thumbnail, and the list needs neither.
        return repository.findHistoryEntries(userId, PageRequest.of(0, AnalysisService.HISTORY_PAGE_SIZE));
    }
